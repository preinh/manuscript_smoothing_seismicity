...
% Intraplate seismicity is a hard problem to solve in tectonics. In general, the earthquake occurrence is lower than others ``active'' regions. In the Brazilian case, there is a lack of observed seismicity due the short (in geological scale) observation timeframe. In addition, the populational density decreases towards to interior of the country and changes the reporting capabilities historicaly. 

% This diffused intraplate seismicity pattern is often modeled as polygons representing different seismic zones or area sources for \gls{psha} purposes. Their geometries are defined by expert opinions.

% Other possible approach to deal this is develop zoneless or smoothed  models for the seismicity, conside
...
All the hazard computations was performed on \gls{oq} \citep{pagani_2014} open source software from \acrfull{gem} Foundation. Until the present moment, no focal mechanism information has been included or considered on these smoothed seismicity models. All hazard computations has been done using a single \gls{GMPE}. A more detailed study on \glspl{GMPE} selection should be developed further and is out of this paper scope.
...
%Tectonic connections between these seismicity models and stress release could be developed in the future.
