...
(not indiscriminately REFERENCE PAGANI WEATHERILL) as a grid of points seismic sources.
...
All the hazard calculations was performed on OpenQuake \citep{pagani_2014} open source software also from GEM Foundation.
...
\subsection{Smoothing Seismicity}
...
Figure \ref{fig_overview} shows the distribution of depths, day of the week and origin hour \citep{woessner_2010-1}. Most events have no depth determination and the well determined depths (from $pP$ phases or local networks) are mostly less than 10km.
...
For this reason, the spatial completeness was not considered and the temporal completeness parameters for the whole country was defined by the simplest expert criteria. 

Figure COMPLETENESS shows the magnitude time distribution and the used completeness table. We chosen just one time and magnitude cut. 
...
\begin{figure}
	...
	\begin{table}[H]
		...
		\begin{tabular}{ c c }
		\footnotesize(a)
        & \includegraphics[width=0.45\textwidth]{z_img_completeness_temporal_stepp}
		\\
		\footnotesize(b)
		& \includegraphics[width=0.45\textwidth]{z_img_completeness_temporal_scatter}
		\end{tabular}
	\end{table}
	\caption{Completeness evaluation. 
	Magnitude-Time scatter plot (a) and the Stepp plot (b).}
