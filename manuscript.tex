...
\authoraddr{Corresponding author: Marlon Pirchiner, Department of Geophysics, IAG, USP, S\~ao Paulo, Brazil. (marlon.pirchiner@iag.usp.br)}
...
\title{Smoothing seismicity techniques applied to seismic source characterization and probabilistic seismic hazard analysis in Brazil}
...
\altaffiltext{1}{Department of Geophysics, Institute of Astronomy, Geophysics and Atmospherical Sciences (IAG), University of São Paulo (USP), São Paulo, Brazil.}
...
This last one was proposed initially to take care of the background seismicity rate in the context of long-term earthquake forecast and the time-independent assumption about the annual background seismic rate could be used
...
The Brazilian seismic hazard building code (REFERENCE) [ABNT] was based on the GSHAP results for South America, which is the last published model for the whole region. However, some significant seismic areas (Porto dos Gaúchos [REFERENCIA]), are not represented in the GPSHAP-based building codes.
...
The bandwidths are defined locally. Could be small in high earthquake density regions and higher where earthquakes are rarely or regions with information lack.
...
If the earthquake occurrence could be modeled by a Poisson process with rate $N_p$, then the probability to observe exactly $n$ events on the consider time frame is
\begin{equation}
	\ensuremath{
...
% \section{If I'll append something}
...
\begin{figure}
	...
	\begin{table}[H]
		...
		\end{tabular}
	\end{table}
	\caption{Catalogue overview. The histogram (a) shows the depth distribution. The histograms (b) and (c) represent the distributions of weekday and hour which earthquakes occur respectively. Dashed lines represent the mean value.}
	\label{fig_overview}
	\end{center}
\end{figure}
...
\begin{figure}
    ...
    \begin{table}[H]
        ...
        \end{tabular}
    \end{table}
    \caption{(a) $h_i$ and (b) $d_i$ distributions for BSB2014.}
    \label{fig_hidi_distributions}
    \end{center}
\end{figure}
...
\begin{figure}
	...
	\begin{table}[H]
		...
		\end{tabular}
	\end{table}
	\caption{Smoothed rates comparison: (a) shows \citet{frankel_1995} method smoothed rate results, (b) and (c) represent rates smoothed by \citet{woo_1996} and \citet{helmstetter_2012} methods respectively.}
	\label{fig_rates}
	\end{center}
\end{figure}
...
\begin{figure}
	...
	\begin{table}[H]
		...
		\end{tabular}
	\end{table}
	\caption{PGA (poe 10\%/50y) comparison for (a) \citet{frankel_1995}, (b) \citet{woo_1996} and (c) \citet{helmstetter_2012} methods.}
	\label{fig_pga}
	\end{center}
\end{figure}
...
\begin{figure}
	...
	\begin{table}[H]
		...
		\end{tabular}
	\end{table}
	\caption{Previous available Brazilian PSHA from (a) GSHAP project and the PSHA proposed by this work (b). }
	\label{placeholder}
	\end{center}
\end{figure}
