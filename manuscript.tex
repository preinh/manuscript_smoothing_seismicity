...
\titlerunninghead{Brazilian Smoothing Seismicity and PSHA}

%------------------------------------------------

% Corresponding author mailing address and e-mail address:
\authoraddr{Corresponding author: Marlon Pirchiner, Department of Geophysics, IAG, USP, São Paulo, Brazil.
...
Institute of Astronomy, Geophysics and Atmospherical Sciences, 
University of São Paulo, São Paulo, Brazil.}

\altaffiltext{2}{School of Applied Math, Getúlio Vargas Foundation, Rio de Janeiro, Brazil.}
...
Intraplate seismicity is a hard problem to solve in tectonics, main during the lack of observed seismicity in
many areas where seismic hazard assesment are also difficult and the methods applied
are often close correlated to the geometries defined by the experts, of course, with valuable criteria.

Three smoothing seismicity techniques are reviewed in order to investigate their appliability to the 
Brazilian seismicity and seismic hazard assesment case. 
All these methods are \emph{catalog-based} in contrast to standard zoning \emph{expert-based} methods from Cornell and
McGuire. Of course it does not mean that the expert there is no value and many parameters needs to be configured.

All the methods are based on a kernel density estimation, but the main diffence between them is the
\emph{bandwidth selection} process. The Frankel method use a \emph{fixed} bandwidth in space, Woo method is based on
\emph{magnitude} also and the Helmstetter is a \emph{local-adaptive} in space and time bandwidth selection. This last
mathod was proposed originally to care the background seismicity rate on long-term earthquake forecast but the one year
forecast is exactly the annual seismic rate used to define a seismic source under a time-independent assumption.

In the conclusions the results shown that all methods gives good results and could be used in the Brazilian
seismicity case, all of them, improoving the spatial resolution of the actual state-of-art GSHAP model. 
The Last two methods was implemented and are available as a free software on HMTK from GEM Foundation. All the hazard
calculations was performed on OpenQuake open source software.
