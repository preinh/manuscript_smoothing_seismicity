...
main during the lack of observed seismicity in many areas where seismic 
...
All the methods are based on a kernel density estimation. The main 
difference between them is the \emph{bandwidth selection} process. 

The Frankel method use a \emph{fixed} bandwidth in space. Woo method
suggest a bandwidth as function of the \emph{magnitude}.
Helmstetter and Werner proposes a \emph{local-adaptive} 
in space and time bandwidth, chosen for each event. 
It was proposed initialy to take care of the background seismicity rate 
on long-term earthquake forecast context, but the one year forecasted 
seismic rate could be a grid of points seismic sources under a 
time-independent assumption.

In the conclusions the results shown that 

all methods gives good results and 
could be used in the Brazilian seismicity case, 

all of them, 
% ###### improoving #########
the spatial resolution of the actual state-of-art GSHAP model.

The Last two methods was implemented and are available as a free software on HMTK from GEM Foundation. 
...
\subsection{Brazilian Seismic Catalog}
Boletim sismico brasileiro.

História e versão com Mw.
\subsection{Brazilian Seismic Catalog}
